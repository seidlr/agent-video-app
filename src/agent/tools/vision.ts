...
/**
 * Core logic behind `segment`, extracted so a human-driven UI action (click-to-segment) can call
 * the exact same pipeline the tool uses without going through the registry -- the Activity feed
 * and its Toast are reserved for what an *agent* did (see Clips.tsx/Frames.tsx's own comments on
 * this same convention), not a running log of human clicks.
 */
export async function runSegment(store: StudioStore, args: SegmentArgs): Promise<ToolResult> {
...
/** Core logic behind `track` -- see runSegment's own doc comment for why this is a standalone,
 * reusable function rather than a registry-only closure. */
export async function runTrack(store: StudioStore, args: TrackArgs): Promise<ToolResult> {
  const state = store.getState();
  if (!state.source) return { ok: false, error: 'no_video_loaded' };
  if (state.source.kind === 'youtube') {
...
/** Core logic behind `detect_scenes` -- see runSegment's own doc comment. */
export async function runDetectScenes(store: StudioStore, args: { addChapters?: boolean; sensitivity?: number; minSceneDuration?: number }): Promise<ToolResult> {
  const state = store.getState();
  if (!state.source) return { ok: false, error: 'no_video_loaded' };
  if (state.source.kind === 'youtube') {
...
/** Core logic behind `detect_objects` -- see runSegment's own doc comment. */
export async function runDetectObjects(
  store: StudioStore,
  args: { time?: string | number; labels?: string[]; threshold?: number; addBoxes?: boolean; confirmDownload?: boolean },
): Promise<ToolResult> {
  const state = store.getState();
  if (!state.source) return { ok: false, error: 'no_video_loaded' };
  if (state.source.kind === 'youtube') {
...
export function defineVisionTools(registry: Registry, store: StudioStore): void {
  registry.define<SegmentArgs>({
    ...
    when: 'local',
    mode: 'job',
    handler: (args) => runSegment(store, args),
  });
  ...
  registry.define<TrackArgs>({
    ...
    when: 'local',
    mode: 'job',
    handler: (args) => runTrack(store, args),
  });
  ...
  registry.define<{ addChapters?: boolean; sensitivity?: number; minSceneDuration?: number }>({
    ...
    when: 'local',
    mode: 'job',
    handler: (args) => runDetectScenes(store, args),
  });
  ...
  registry.define<{ time?: string | number; labels?: string[]; threshold?: number; addBoxes?: boolean; confirmDownload?: boolean }>({
    ...
    when: 'local',
    mode: 'job',
    handler: (args) => runDetectObjects(store, args),
  });
