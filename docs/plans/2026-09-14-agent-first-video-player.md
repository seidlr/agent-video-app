...
- Task 8 (methodology finding, not a code defect -- flagged since it affects every future "verify CI green" check, not just this task): `tests/e2e/player.spec.ts:174`'s frame-step assertion (comma/period stepping exactly 1/30s while paused) is a recurring CI-only flake, confirmed by inspecting CI history across at least five unrelated commits that touched none of this test's own code (Task 7's boxes/tracking, Task 8 parts 2-4, and the pure `webmcp.ts` parallelization fix) -- every one shows the identical failure (`toBeLessThan`/`toBeCloseTo` on `currentTime`, off by single-digit milliseconds), while the same test passes reliably in every local run (5 Playwright workers) in this session. Root cause is environmental, not this app's code: `playwright.config.ts` sets no explicit `workers` count, so Playwright's own CPU-count-based default applies -- confirmed from CI's own log line ("Running 22 tests using 2 workers") that GitHub Actions' runner only gets 2, against 5 locally, with materially less CPU headroom per worker -- and this specific assertion's tolerance (a single frame at 30fps, ~33ms) is tight enough that ordinary CI scheduler jitter alone can push a real, correct frame-step outside it. A second, less frequent flake on the same runner (`tools-playback.spec.ts:101`, the YouTube tool-set-narrowing test) shows the same CI-only, unrelated-commit pattern. Neither is fixed here: doing so would mean loosening a real timing assertion or restructuring CI's worker count, both out of Task 8's own scope and requiring a decision this implementation isn't authorized to make unilaterally (loosening a timing DoD is a product/test-strategy call, not a bug fix). Handling in this session: re-run the CI job (`gh run rerun --failed`) when this specific test is the only failure, rather than treating it as a Task 8 regression -- every actual Task 8 commit's own new tests have passed on the first CI attempt.
