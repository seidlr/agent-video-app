...
- Task 9 (material, discovered before any Task 9 file was touched -- surfaced here rather than silently expanded or silently deferred again): the plan's own top-level Verification bullet #2 reads "After a reload ... the last project, its video, frames, boxes, notes and transcript are all still present and playable." No task's own Definition of Done actually implements this. Direct inspection found: `store/frames.ts`'s `persistFrame` and `store/boxes.ts`'s `persistBox` each already carried their own "not yet wired to restore ... across a reload" doc comment (Tasks 5 and 7, respectively, consciously deferring this); `notes`/`chapters`/`clips` had reserved Dexie tables since Task 2's scaffolding but zero persistence code of any kind (never written, let alone restored); `transcript` was written (Task 8's `persistTranscript`) but never read back on boot; and `add_box`'s own plain CRUD path (`agent/tools/boxes.ts`) never called the already-existing `persistBox` at all -- only the ML `segment` path did. Every one of Tasks 5-8's own DoD lines is still satisfied as written; this gap sat entirely outside all of their stated scopes, which is exactly why it went unfixed for four tasks running. Rather than build Task 9's brand-new `clips` feature with the identical defect, or leave four already-"complete" tasks silently short of the plan's own headline acceptance bullet, implemented the fix as Task 9's own first unit of work: a single `store/projectPersistence.ts` module using one `store.subscribe((state, prev) => ...)` listener (not a persistence call added to all sixteen individual add/update/remove/clear/reorder actions across notes/chapters/boxes/tracks/clips, which would have needed to be repeated at every tool AND UI call site and risks silently missing one) -- each slice is always wholesale-replaced by its own action, so reference inequality against the previous state cheaply detects "did this change" with no deep diff. Boxes needed their own sync function (not the generic one used for notes/chapters/tracks/clips): the Dexie `boxes` row carries an extra `maskBlob` field the in-memory `Box` never has, so a plain `put()` on every edit would silently erase a previously-segmented mask the instant any other field (label, position) changed; `db.boxes.update()` merges instead, preserving `maskBlob` untouched, and only a genuinely new box (nothing to preserve) goes through `put()`. Added five new bulk-replace setters (`setNotes`/`setChapters`/`setBoxes`/`setTracks`/`setClips`/`setFrames`, matching Task 8's own `setScenes`/`setTranscript` wholesale-replace convention) so restoring a persisted row keeps its original id, unlike calling `addX` in a loop (which mints a fresh id every time via `nextId()`). `store/frames.ts` gained a symmetrical `restoreFrames` (fresh `URL.createObjectURL` per row, since object URLs never survive a reload, exactly as `FrameEntry`'s own doc comment already said they should be). `main.tsx` now runs `restoreProjectData`+`restoreFrames` at boot (asset-agnostic, since this is a single-project studio), arms `wireProjectPersistence` only after that restore resolves (so the restore's own setter calls don't trigger a harmless-but-wasteful write-back), and separately restores the transcript once the source-restore resolves an `assetId` (transcript is the one slice keyed per-asset). Live-verified end to end with a real reload (`tests/e2e/persistence.spec.ts`): notes, chapters, boxes, and frames added via WebMCP tools are still present -- in both the tool-level JSON and the rendered Notes panel -- after `page.reload()` with no re-selection of the file, and (separately, `@ml`-tagged since it needs a real Whisper download) a transcribed segment is still returned by `get_transcript` after reload too. Fully unit-tested (`tests/unit/projectPersistence.test.ts`, `store-frames.test.ts`'s new `restoreFrames` case, `studio-store.test.ts`'s new bulk-setter case) including the specific box/maskBlob-preservation scenario. Boot-time restore is best-effort, same tolerance this app already applies to the existing `getLastSource`-driven source restore.
- Task 9 (tactical, scope decision): `media/project.ts`'s `export_project`/`import_project` round trip is metadata-only -- `includeMedia` (bundling the source video itself into the zip via a streaming `Zip`/`ZipPassThrough`, per the plan's own Key Decisions) is deferred. Neither TS-007 step 4's own literal test ("Download a .zip containing `project.json`, `frames/*.png`, `notes.md`") nor this task's own DoD line ("restores notes, chapters, boxes, clips, frames byte-identically") exercises it, and correctly streaming a multi-hundred-MB video through `fflate`'s async `Zip` API (rather than the synchronous `zipSync` metadata export uses) is a separately-verifiable unit of work in its own right. SHORTCUT, upgrade trigger: a real request to move a whole project, video included, between profiles. Separately, `project.json`'s schema needed one addition beyond the plan's literal wording ("schema from Task 6 JSON export + tracks/clips/thumbnail vtt"): a `frames` metadata array (id/time/kind/width/height/file), since nothing else in that schema says which zip-relative PNG belongs to which frame, at what time -- without it, "restores ... frames byte-identically" would have no way to reconstruct `FrameRow`s on import at all. The "thumbnail vtt" part of that same sentence (the sprite hover-preview VTT) was *not* added -- it belongs to the filmstrip preview, isn't named in the DoD's own restoration list, and isn't needed to satisfy it. `import_project` itself is not a registered agent tool (the plan's own Files list only names `export_project` for `agent/tools/exports.ts`): a multi-MB zip is an awkward fit for the JSON tool-call contract WebMCP tools use, so it's a plain function (`applyImportedProject`) wired to the Library panel's new "Import project" file input instead, matching the Files list's own separate "Modify: ... Library.tsx (Import project button)" entry. Live-verified end to end (`tests/e2e/project.spec.ts`) across two genuinely separate Playwright browser contexts (their own IndexedDB/OPFS each, reproducing the DoD's own "on a fresh profile" wording): a real `<a download>` zip from one context, fed through the second context's file input, restores the exact note/chapter/box/clip/frame added in the first.
